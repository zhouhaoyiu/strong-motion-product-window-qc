...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=5.0,
        help="Append this many zero-input oscillator cycles before taking the PSA maximum.",
    )
...
def load_record_waveform(
    row: pd.Series,
    instance_data,
    h5,
    keys: set[str] | None,
    knet_highpass_hz: float | None,
    pnw_data=None,
    ...
    elif dataset == "K-NET" and knet_highpass_hz:
...
def pseudo_spectral_acceleration(
    ...
    ringdown_samples = max(0, int(np.ceil(float(ringdown_cycles) * float(period) * float(sampling_rate))))
    if ringdown_samples:
        clean = np.pad(clean, (0, ringdown_samples))
    displacement = lfilter(b, a, clean)
    return float((omega * omega) * np.max(np.abs(displacement)))


def max_component_psa(
    waveform: np.ndarray,
    period: float,
    damping: float,
    sampling_rate: float,
    ...
    values = component_psa_values(waveform, period, damping, sampling_rate, ringdown_cycles)
    finite = values[np.isfinite(values)]
    return float(np.max(finite)) if finite.size else float("nan")


def component_psa_values(
    waveform: np.ndarray,
    period: float,
    damping: float,
    sampling_rate: float,
    ...
    return np.asarray(
        [
            pseudo_spectral_acceleration(
                arr[channel], period, damping, sampling_rate, ringdown_cycles=ringdown_cycles
            )
...
def record_spectrum_rows(
    ...
    full_by_period = {
        period: component_psa_values(
            waveform,
            period=period,
            damping=damping,
            sampling_rate=sampling_rate,
            ringdown_cycles=ringdown_cycles,
        )
    ...
    for _, window_row in windows.iterrows():
        ...
        for period in periods:
            ...
            if key not in cache:
                cache[key] = component_psa_values(
                    segment,
                    period=period,
                    damping=damping,
                    sampling_rate=sampling_rate,
            ...
            full_components = full_by_period[period]
            window_components = cache[key]
            valid = np.isfinite(full_components) & (full_components > 0)
            ratios = np.ones_like(full_components, dtype=float)
            ratios[valid] = np.where(
                np.isfinite(window_components[valid]),
                window_components[valid] / full_components[valid],
                0.0,
            )
            retention = float(np.min(ratios)) if ratios.size and valid.any() else float("nan")
            full_psa = float(np.nanmax(full_components)) if np.isfinite(full_components).any() else float("nan")
            window_psa = float(np.nanmax(window_components)) if np.isfinite(window_components).any() else float("nan")
            rows.append(
                {
                    ...
                    "psa_retention_definition": "minimum_component_psa_retention",
                    "ringdown_cycles": float(ringdown_cycles),
...
def write_report(
    ...
    lines = [
        ...
        "",
        f"Each oscillator response is continued for {ringdown_cycles:g} zero-input cycles after the waveform ends so that a window boundary does not truncate free vibration.",
        "",
...
def run_response_spectrum_retention(
        ...
        for _, row in tqdm(records.iterrows(), total=len(records), desc="Response spectra"):
            ...
            try:
                waveform = load_record_waveform(
                    ...
                    knet_highpass_hz,
                    pnw_data=pnw_data,
                    acceleration_highpass_hz=acceleration_highpass_hz,
                )
                rows.extend(
    ...
    write_report(outdir, summary, period_values, policy_values, ringdown_cycles)
    if load_errors:
...
def main() -> None:
    ...
    outputs = run_response_spectrum_retention(
        ...
        ringdown_cycles=args.ringdown_cycles,
