...
from scripts.compute_strong_motion_qc_features import (  # noqa: E402
    ...
    preprocess_acceleration_waveform,
    standardize_channels,
)
from strong_motion_qc.onset import energy_fraction_time
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--acceleration-highpass-hz",
        type=float,
        default=None,
        help="Apply linear detrending and this high-pass corner to every acceleration record.",
    )
    ...
    parser.add_argument("--fixed-after-grid", nargs="*", type=float, default=[])
...
def candidate_windows(
    ...
    fixed_after_grid: list[float] | None = None,
    signal: np.ndarray | None = None,
    arias_padding_sec: float = 15.0,
    ...
    feature_onset = pd.to_numeric(pd.Series([row.get("feature_onset_sec", np.nan)]), errors="coerce").iloc[0]
    for after_sec in fixed_after_grid or []:
        if np.isclose(after_sec, fixed_after_sec):
            continue
        suffix = f"{after_sec:g}".replace(".", "p")
        candidates.append(
            {
                "candidate": f"feature_onset_fixed_{suffix}s",
                "start_sec": float(feature_onset - pre_sec) if np.isfinite(feature_onset) else float("nan"),
                "end_sec": float(feature_onset + after_sec) if np.isfinite(feature_onset) else float("nan"),
            }
        )
    ...
    if signal is not None:
        t01 = energy_fraction_time(signal, sampling_rate, 0.01)
        t99 = energy_fraction_time(signal, sampling_rate, 0.99)
        candidates.append(
            {
                "candidate": "arias_1_99_padded",
                "start_sec": float(t01 - arias_padding_sec),
                "end_sec": float(t99 + arias_padding_sec),
            }
        )
...
def evaluate_window(
    ...
    if components is None:
        ...
        component_array = standardize_channels(normalize_waveform_orientation(components))
        if component_array.shape[-1] != signal.size:
            raise ValueError("component and vector-signal lengths differ")
        full_component_peaks = np.max(np.abs(component_array), axis=1)
        window_component_peaks = (
            np.max(np.abs(component_array[:, start:end]), axis=1)
            if end > start
            else np.full(component_array.shape[0], np.nan)
        )
        valid = np.isfinite(full_component_peaks) & (full_component_peaks > 0)
        component_retention = np.ones(component_array.shape[0], dtype=float)
        component_retention[valid] = window_component_peaks[valid] / full_component_peaks[valid]
        pga_retention = float(np.nanmin(component_retention)) if component_retention.size else float("nan")
        peak_samples = np.argmax(np.abs(component_array), axis=1)
        peak_inside = bool(np.all((~valid) | ((peak_samples >= start) & (peak_samples < end))))
        full_peak = float(np.max(full_component_peaks))
        window_peak = float(np.nanmax(window_component_peaks)) if np.isfinite(window_component_peaks).any() else float("nan")
    ...
    return {
        ...
        "full_pga": full_peak,
        "window_pga": window_peak,
        "pga_retention": float(pga_retention),
        "peak_retention_definition": "minimum_component_peak_acceleration_retention" if components is not None else "vector_peak_retention",
...
def load_waveforms_for_rows(features: pd.DataFrame, knet_waveforms: Path):
    ...
    if features["dataset"].eq("InstanceGM").any():
        ...
        instance_data = sbd.InstanceGM(component_order="ZNE")
    pnw_data = None
    if features["dataset"].eq("PNWAccelerometers").any():
        import seisbench.data as sbd

        pnw_data = sbd.PNWAccelerometers(component_order="ENZ")
    h5 = None
    keys = None
...
def evaluate_record(
    ...
    components = standardize_channels(normalize_waveform_orientation(waveform))
    signal = vector_signal(components)
    rows = []
    for candidate in candidate_windows(
        ...
        fixed_after_sec,
        adaptive_post_sec,
        fixed_after_grid=fixed_after_grid,
        signal=signal,
    ):
...
def write_report(outdir: Path, summary: pd.DataFrame, results: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "",
        "The endpoints are the minimum component peak-acceleration retention, three-component squared-motion retention, and inclusion of every component peak time.",
        "",
        ...
        "## Interpretation",
        "",
        "A failed window requires a longer candidate or a product-specific safeguard before the record is exported.",
    ]
    if len(unstable):
        top = (
...
def run_window_stability(
    ...
    acceleration_highpass_hz: float | None = None,
    ...
    try:
        for _, row in tqdm(work.iterrows(), total=len(work), desc="Evaluate windows"):
            try:
                ...
                if acceleration_highpass_hz is not None:
                    waveform = preprocess_acceleration_waveform(
                        waveform,
                        sampling_rate=float(row["sampling_rate_hz"]),
                        corner_hz=acceleration_highpass_hz,
                    )
                elif row["dataset"] == "K-NET" and knet_highpass_hz:
...
def main() -> None:
    ...
    outputs = run_window_stability(
        ...
        acceleration_highpass_hz=args.acceleration_highpass_hz,
        max_records=args.max_records,
        per_group=args.per_group,
        seed=args.seed,
        pre_sec=args.pre_sec,
        fixed_after_sec=args.fixed_after_sec,
        fixed_after_grid=args.fixed_after_grid,
