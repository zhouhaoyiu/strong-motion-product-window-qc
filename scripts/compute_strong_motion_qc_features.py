...
def preprocess_acceleration_waveform(waveform: np.ndarray, sampling_rate: float, corner_hz: float) -> np.ndarray:
    from scipy.signal import detrend

    arr = normalize_waveform_orientation(waveform)
    arr = detrend(arr, axis=-1, type="linear")
    return highpass_waveform(arr, sampling_rate=sampling_rate, corner_hz=corner_hz)
...
def compute_row(
    ...
    try:
        ...
        if acceleration_highpass_hz is not None:
            ...
            preprocess_name = f"linear_detrend_highpass_{acceleration_highpass_hz:g}hz"
            ...
            preprocess_name = f"knet_highpass_{knet_highpass_hz:g}hz"
        else:
            preprocess_name = ""
        ...
        result.update(
            {
                ...
                "waveform_preprocess": preprocess_name,
            }
        )
...
def write_report(outdir: Path, features: pd.DataFrame, summary: pd.DataFrame) -> None:
    lines = [
        ...
        "",
        "Catalog P/S fields remain diagnostic columns. Feature values are derived from waveform samples.",
        "",
        "## Summary",
        "",
        ...
        "## Next Step",
        "",
        "Use loaded records with acceptable waveform-QC flags to construct candidate windows and evaluate product retention by dataset and magnitude group.",
    ]
