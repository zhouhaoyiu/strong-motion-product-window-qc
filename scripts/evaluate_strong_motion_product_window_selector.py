...
FIXED_POLICIES = {
    ...
    "arias_1_99_padded": "arias_1_99_padded",
...
def eligible_nonfull_candidate(candidate: object, include_catalog: bool = False) -> bool:
    name = str(candidate)
    return (
        name.startswith("feature_onset_fixed")
        or name in {"energy_onset_fixed", "feature_onset_to_energy_end", "arias_1_99_padded"}
        or (include_catalog and name == "catalog_p_fixed")
    )


def choose_shortest_stable(group: pd.DataFrame, include_catalog: bool = False) -> tuple[pd.Series | None, str]:
    eligible = group["candidate"].map(lambda value: eligible_nonfull_candidate(value, include_catalog))
    subset = group[eligible].copy()
...
def choose_policy(group: pd.DataFrame, policy: str) -> tuple[pd.Series | None, str]:
    ...
    if policy == "shortest_stable_no_catalog":
        return choose_shortest_stable(group, include_catalog=False)
    if policy == "shortest_stable_all":
        return choose_shortest_stable(group, include_catalog=True)
...
def write_report(outdir: Path, summary: pd.DataFrame, usage: pd.DataFrame) -> None:
    ...
    lines = [
        ...
        "",
        "The selector applies prespecified waveform-product checks against the available full record.",
        "",
        ...
        "",
        "`shortest_stable_*` policies are offline product-stability selectors computed with the available full record. Their outputs are processing windows and audit fields for archive or batch-product preparation.",
    ]
