...
def write_report(outdir: Path, manifest: pd.DataFrame, summary: pd.DataFrame, mag_summary: pd.DataFrame) -> None:
    lines = [
        ...
        "",
        "This manifest is the all-magnitude metadata entry point for StrongMotion-QC.",
        "",
        "Waveform loading and QC follow manifest construction. The manifest separates metadata-level waveform candidates and descriptive magnitude groups.",
        "",
        "Catalog P/S fields are retained for diagnostic comparisons and do not enter the primary window selector.",
        "",
        "## Dataset Summary",
        "",
...
def load_pnw_metadata() -> pd.DataFrame:
    ...
    data = sbd.PNWAccelerometers(component_order="ENZ")
    return data.metadata
