...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--acceleration-only",
        action="store_true",
        help="Keep acceleration records only; InstanceGM velocity records are excluded.",
    )
...
def acceleration_record_mask(manifest: pd.DataFrame) -> pd.Series:
    dataset = manifest["dataset"].astype(str)
    units = manifest.get("units", pd.Series("", index=manifest.index)).astype(str).str.lower()
    explicit_acceleration = units.isin({"mps2", "m/s2", "cm/s2", "gal", "acceleration", "accelerometer"})
    known_acceleration_archive = dataset.isin({"K-NET", "PNWAccelerometers"})
    return explicit_acceleration | known_acceleration_archive
...
def build_worklist(
    ...
    if acceleration_only:
        pool = pool[acceleration_record_mask(pool)].copy()
    if pool.empty:
...
def write_report(outdir: Path, worklist: pd.DataFrame, priority_summary: pd.DataFrame) -> None:
    lines = [
        ...
        "",
        "This is a stratified waveform-loading worklist drawn from the all-magnitude metadata manifest. It defines the records entering waveform QC and processing-window evaluation.",
        "",
        "The worklist includes low-magnitude background records, M3-M4 small-event records, and M4+ strong-motion records. Catalog P and S fields remain evaluation-only columns.",
        "",
        "## Counts",
        "",
        ...
        "## Next Step",
        "",
        "Load waveform records from this worklist, compute onset and QC features, construct candidate windows, and evaluate product retention.",
    ]
...
def build_from_manifest(
    ...
    worklist = build_worklist(
        ...
        acceleration_only=acceleration_only,
    )
...
def main() -> None:
    ...
    outputs = build_from_manifest(
        ...
        acceleration_only=args.acceleration_only,
    )
