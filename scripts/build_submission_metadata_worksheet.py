...
FIELD_GUIDANCE = {
    ...
    "corresponding_author_name": (
        "通讯作者姓名",
        "核对负责投稿、修回和校样联系的通讯作者姓名。",
        "Full Name",
        "title page; submission system",
    ),
    "corresponding_author_email": (
        "通讯作者邮箱",
        "核对通讯作者可长期接收投稿系统邮件的邮箱。",
        "name@example.edu",
        "title page; submission system",
    ),
    "corresponding_author_mailing_address": (
        "通讯作者邮寄地址",
        "核对通讯作者的完整邮寄地址。",
        "Department, Institution, Street, City, Postal Code, Country",
        "title page",
    ),
    "funding_statement": (
        "基金声明",
...
def write_worksheet(
    ...
    with csv_path.open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(
            handle,
            fieldnames=list(WorksheetItem.__dataclass_fields__.keys()),
            lineterminator="\n",
        )
        writer.writeheader()
        for item in items:
